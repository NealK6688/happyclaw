...
- **输出**：飞书富文本卡片 / Telegram HTML / QQ 纯文本 / WeChat 加密消息 / 钉钉 AI Card / Discord embed / Web 实时流式推送
- **记忆**：Agent 自主维护 `CLAUDE.md` / `AGENTS.md` 和工作区文件，实现跨会话持久记忆
...
| `src/billing.ts` | 计费访问控制 + 配额校验 + 30s access/quota 双重 cache（详见 `routes/billing.ts`） |
| `src/wechat.ts` + `src/wechat-crypto.ts` | 微信公众号通道：被动消息 + 加密推送（AES + Token 校验） |
| `src/discord.ts` + `src/discord-streaming-edit.ts` | Discord 通道：Gateway WebSocket + 流式 edit 编辑模式 |
| `src/feishu-mention-gate.ts` | 飞书群聊 @机器人 mention 门控（per-group `require_mention`） |
| `src/feishu-streaming-card.ts` / `dingtalk-streaming-card.ts` / `qq-streaming-card.ts` | 4 套渠道独立 streaming card 实现（STATUS_BANNER + Todo + 工具时间轴 + 思考过程 + 调用轨迹 5 面板，共 ~10k 行，**未抽象**） |
| `src/dingtalk-reply-parser.ts` | 钉钉 reply event 解析（带 inline content / mention 处理） |
| `src/im-context-isolation.ts` | per-user IM jid → 主容器 folder 映射（确保跨用户隔离） |
| `src/agent-capabilities.ts` | Agent 能力枚举（如 agent-browser / Vision / Web search 等运行时探测） |
| `src/provider-pool.ts` | Claude provider 池：session-sticky 绑定（避免跨 OAuth 账号 thinking block 签名失效） |
| `src/runtime-owner.ts` | 运行时主用户解析（admin 与 member 主容器区分） |
| `src/cross-group-acl.ts` | 跨组操作 ACL：admin 主容器跨群组消息发送 / 任务管理白名单 |
| `src/workspace-config.ts` | 工作区级配置加载（per-folder 覆盖系统默认） |
| `src/agent-definitions.ts` | 预定义 SubAgent 注册（与 agent-runner 同名文件配合） |
| `src/task-routing.ts` | 任务回流路由：`broadcastToOwnerIMChannels` 把任务输出广播到对应 owner 的 IM |
| `src/task-utils.ts` | 任务工具函数（cron 计算、上下文构造、状态机辅助） |
| `src/message-notifier.ts` | 消息通知器：跨 IM 渠道的通知派发 |
| `src/bug-report.ts` | Bug 反馈：用户报 bug 时收集日志快照 |
| `src/plugin-catalog.ts` | Plugin v3 共享 catalog（admin scan，content-hash 寻址） |
| `src/plugin-materializer.ts` | Plugin runtime materialize（COPYFILE_FICLONE 接近零拷贝 + marker rollback） |
| `src/plugin-importer.ts` | Plugin 导入逻辑（从宿主机 / git URL / 文件解压） |
| `src/plugin-manifest.ts` | Plugin manifest 解析与校验 |
| `src/plugin-utils.ts` | Plugin 加载工具：`loadUserPlugins(userId, {runtime})` → `SdkPluginConfig[]` |
| `src/plugin-command-index.ts` | Plugin commands 索引（用于斜杠命令补全） |
| `src/plugin-expander-core.ts` 等 5 个 plugin-expander | Plugin context expander（store / sentinel / context / core），处理 plugin 自定义 prompt 展开 |
| `src/plugin-dependency-check.ts` | Plugin 依赖 best-effort 预检（命令第一 token + override 表） |
| `src/feishu-cards/` | 飞书 V2 卡片 JSON 模板集合 |
| `src/mcp-utils.ts` | MCP server 加载工具（`loadUserMcpServers()`：从 `mcp-servers/{userId}/servers.json` 读取并转 SDK 配置） |
...
- **流式事件**（14 类）：`text_delta`、`thinking_delta`、`tool_use_start/end`、`tool_progress`、`hook_started/progress/response`、`task_start`、`task_notification`、`todo_update`、`usage`、`status`、`init` —— 通过 WebSocket `stream_event` 消息广播到 Web 端，类型源定义见 `shared/stream-event.ts`
- **文本缓冲**：`text_delta` 累积到 200 字符后刷新，避免高频小包
- **会话循环**：`query()` → 等待 IPC 消息 → 再次 `query()` → 直到 `_close` sentinel
- **MCP Server**：17 个工具，通过 SDK `createSdkMcpServer()` 以同进程模式注册
  - **消息发送**（3）：`send_message`、`send_image`、`send_file`
  - **定时任务**（5）：`schedule_task`、`list_tasks`、`pause_task`、`resume_task`、`cancel_task`
  - **跨组管理**（1）：`register_group`（admin 主容器特权）
  - **Discord 专属**（3）：`discord_get_history`、`discord_get_channel_info`、`discord_get_server_info`
  - **Skill 管理**（2）：`install_skill`、`uninstall_skill`
  - **跨会话记忆**（3）：`memory_append`、`memory_search`、`memory_get`
...
| `mcp-tools.ts` | MCP 工具定义：17 个工具通过 SDK `tool()` 注册，IPC 文件通信 |
...
SQLite WAL 模式，Schema 经历 v1→v37 演进（`db.ts` 中的 `SCHEMA_VERSION`）。当前 27 张表（不含 migration 临时 `_new` 后缀表）。
...
| `daily_usage` | `(user_id, date)` | 旧版日用量（保留兼容） |
| `monthly_usage` | `(user_id, year_month)` | 月用量聚合（订阅 quota / 计费） |
| `user_quotas` | `user_id` | 用户配额（预留，暂不写入数据） |
| `user_balances` | `user_id` | 用户余额（充值后人民币 / 美元） |
| `user_subscriptions` | `user_id` | 订阅信息（plan_id、started_at、cancel_at） |
| `balance_transactions` | `id` (auto) | 余额变动流水（充值 / 扣费 / 退款） |
| `billing_plans` | `id` | 订阅计划定义（月度 / 年度 / 配额） |
| `billing_audit_log` | `id` (auto) | 计费操作审计 |
| `redeem_codes` | `code` | 兑换码（兑换为余额 / 订阅 / quota） |
| `redeem_code_usage` | `(code, user_id)` | 兑换码使用记录 |
| `im_context_bindings` | `(user_id, jid)` | IM 上下文绑定（per-user 主容器与 IM jid 的映射） |
| `user_pinned_groups` | `(user_id, group_jid)` | 用户置顶群组（侧栏排序） |
